...
OUTPUT_CSV = Path("evaluation_results_TQA.csv")
OUTPUT_JSONL = Path("evaluation_results_TQA.jsonl")
...
#results = []
file_paths = sorted(TESTDOCS_DIR.glob("*.json"))[:10]
...
with open(OUTPUT_CSV, "w", newline="") as f_csv, open(OUTPUT_JSONL, "w") as f_jsonl:
    ...
    for idx, file_path in enumerate(tqdm(file_paths, desc="Evaluating", unit="example")):
        ...
        from langchain.schema import Document
        # Limit to top 3 context passages per example (or even 2 to be safer)
        wrapped_docs = [Document(page_content=txt) for txt in context_paragraphs[:3]]
        ...
        print(f"[{idx+1}/{len(file_paths)}] Memory used: {mem.used / 1024**3:.2f} GB / {mem.total / 1024**3:.2f} GB")
